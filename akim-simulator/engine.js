...
function validate(choices,complete=true){
 ...
 if(choices.length>5)errors.push('Нельзя выбрать более 5 решений.');
 const seen=new Set(),counts={},where={};let cost=0;
 for(const choice of choices){const m=byId[choice?.id];if(!m){errors.push('Неизвестное мероприятие.');continue}
...
function calculate(choices){const errors=validate(choices);if(errors.length)return {valid:false,errors};
 ...
 for(const choice of choices){const m=byId[choice.id],targets=m.type==='Город'?districts.map((_,i)=>i):[districts.findIndex(d=>d.name===choice.district)],factor=(8-m.lag)/8;
 ...
 return {valid:true,cost:choices.reduce((s,c)=>s+byId[c.id].cost,0),score:.7*average+.3*minimum-critical.length,average,minimum,critical,rows,contributions};
}
...
if(typeof module!=='undefined')module.exports={K,W,districts,measures,validate,calculate,baseline};
